...
class TestHugoSiteGeneration:
    ...
    def test_publish_page_creates_content(self):
        """publish_page creates a markdown file in the pages section."""
        from prax.services.workspace_service import _ensure_workspace
        root = _ensure_workspace(USER)

        # Monkeypatch _run_hugo.
        original_run = course_service._run_hugo
        course_service._run_hugo = lambda s: None

        try:
            result = course_service.publish_page(
                USER, "eigenvalues-explained", "Eigenvalues Explained",
        ...
        assert "eigenvalues-explained" in result["url"]

        # Check the file was created.
        site = course_service._hugo_site_dir(root)
        page_path = os.path.join(site, "content", "pages", "eigenvalues-explained.md")
        assert os.path.isfile(page_path)
        with open(page_path) as f:
            content = f.read()
        assert "Eigenvalues Explained" in content
        assert "$$Av = \\lambda v$$" in content

    def test_publish_page_sanitizes_slug(self):
        """Slugs are sanitized to filesystem-safe characters."""
        from prax.services.workspace_service import _ensure_workspace
        ...
        try:
            result = course_service.publish_page(
                USER, "My Fancy Page!!!", "Title", "Body",
                "https://example.ngrok.io",
            )
        finally:
            course_service._run_hugo = original_run

        assert "my-fancy-page" in result["url"]
