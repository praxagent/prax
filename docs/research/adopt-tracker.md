...
| **Sandbox secret-exfil hardening** (egress allowlist/proxy; **dedicated spend-capped sandbox key**; keys out of the shared container env; no-net/read-only DuckDB) | [sandbox-execution-boundary](../security/sandbox-execution-boundary.md) | 📋 | **live exposure** — the container env carries `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` (for OpenCode), readable by any exec tool (`getenv`/`printenv`) + unrestricted egress = a prompt-injection-drivable key-exfil path. Ranked: egress restriction kills it for env *and* files; a dedicated rotatable key bounds the blast radius |
