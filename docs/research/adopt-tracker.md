...
| **Sandbox container egress hardening** (allowlist/proxy; no-net compute mode; DuckDB read-only/no-httpfs) | [sandbox-execution-boundary](../security/sandbox-execution-boundary.md) | 📋 | the residual exfiltration leg — container network egress is unrestricted, reachable by untrusted content driving a code-exec tool. Perimeter trifecta guard + taint reduce the *path*; the raw egress isn't itself restricted |
