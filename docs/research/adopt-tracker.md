...
| **"Executable world-models" — a general model-induction capability** (induce an executable model of an unknown system from evidence → verify vs reality → plan/apply; a `modeling`/`simulate` extension of the sandbox) | [arc-agi-3](arc-agi-3-schema-harness.md) | ⏸ precondition | **the feature, not a "game spoke"** — broadly useful (onboard unknown APIs, empirical debugging, simulation-planning, rule discovery). Land with a **non-ARC** demo so generality is visible. ARC-AGI-2/3 are two *benchmarks* of it. Build after the shakedown runs clean |
...
| **Self-authored ARC task generators** (novel parameterized ARC-2 grid tasks + ARC-3 mini-games, each with a *known* program → deterministic scoring) | [arc-agi-3](arc-agi-3-schema-harness.md) | ⏸ precondition | unlimited, novel, **contamination-free** training/eval signal; trains the *general* capability not memorized answers. Public *train* half = dev, public *eval* half = held-out check, never trained on. **This generator + deterministic verifier = the un-gameable fitness function [#29](../IDEAS_BACKLOG.md) needs** (ties the whole self-regen cluster to a checkable domain) |
