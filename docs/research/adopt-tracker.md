...
| **Secrets proxy — KEYLESS Prax (Tier 1)** (proxy holds keys, injects into OpenAI+Anthropic calls, streams back) | [secrets-proxy](../security/secrets-proxy.md) | ✅ | **built** — `prax/secrets_proxy/`; Prax runs with placeholder keys, nothing to steal (the "make the secret unreachable" wall). Allowlist + audit log; 10 keyless tests + live smoke |
