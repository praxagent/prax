...
| **Silent-failure test suite for `prax-secrets-proxy`** (stateless-per-request under concurrency; stream-integrity; auth isolation) | [failure-atlas](failure-atlas-llm-gateways.md) | 📋 | turn the applicable gateway modes into keyless invariants — pins the proxy's trust-critical *stateless pass-through* design; the natural next increment (reliability story to match the security one) |
| **Record the model that *actually* answered** (not just the requested one) on the failover path | [failure-atlas](failure-atlas-llm-gateways.md) | 📋 | cheapest defense vs **silent model substitution** (`llm_fallback.py` + third-party base URLs); feeds hallucination-guard / provenance |
| **Bound failover retries** (cap + backoff + jitter; 429 fails *over*, never blocks) | [failure-atlas](failure-atlas-llm-gateways.md) | 📋 | stops a provider blip becoming a **retry storm** / rate-limit deadlock in `llm_fallback.py`. Proxy itself has NO retry logic (deliberate) |
