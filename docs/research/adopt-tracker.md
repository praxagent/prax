...
| **ARC-AGI-3 adapter + schema-induction `game` spoke** (executable world-model loop: perceive→induce-schema-as-sandbox-program→verify-vs-reality→plan-by-simulation→act→refine; persist schemas in memory) | [arc-agi-3](arc-agi-3-schema-harness.md) | ⏸ precondition | the flagship interactive eval; **build after the standard-benchmark shakedown runs clean**. `arc_agi_3.py` (flag-gated `ARC_AGI3_ENABLED`, real games in `PRAX_EVAL_DIR`, keyless mock for CI) + a `game` spoke reusing sandbox/codegen/memory. Metric (RHAE, hidden games) is anti-spike by construction |
