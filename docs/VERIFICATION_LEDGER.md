...
| **Forward (MITM) proxy** (all REST egress via `HTTPS_PROXY`, registry-driven injection) | 🟡 | **Verified live 2026-07-22:** the mitmproxy forward proxy runs with the registry-generated forward-map and injects the real key per host — **Serper** (search) and **OpenAI** both return 200 through it with a keyless client, and **Prax's own web search runs keyless through the proxy** (placeholder key → proxy strips + injects). The first live run also caught + fixed a real bug (`secrets_proxy/__init__` eagerly imported Flask/requests → the mitmproxy addon crashed on load). Injector unit-tested (10 tests) + forward-map never-drift test. | Not every provider confirmed: **ElevenLabs** injects correctly (log shows `xi-api-key` set) but returns 401 → the held EL key is stale (rotate). Twilio/Twitter/Threads/Google/Jina not yet exercised live. And it's **not Prax's live default yet** — needs the `.env` wiring (`HTTPS_PROXY` + CA bundle + **placeholder** REST keys, non-empty so Prax's presence-guards don't short-circuit before reaching the proxy). |
