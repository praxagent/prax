...
| Running a real job in the sandbox and collecting its artifacts | 🟡 | A live call reaches the sandbox and returns a real exit code with a full timeline (no crash, no fallback). A shape bug found only by that live run — the client returns `subprocess.CompletedProcess`, not a dict — is fixed and regression-tested. | **A successful end-to-end job with artifacts read back has not been observed.** The live probe exited non-zero and its workspace was not readable, so artifact collection returned empty with `artifact_collection_error` set (honest, but unproven on the happy path). Needs one real prax-lab probe run: non-zero exit → artifacts present → hashes match. Until then treat artifact collection as unverified-live. |
