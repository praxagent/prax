...
| **Forward-proxy caller authentication** (`PROXY_FORWARD_AUTH_TOKEN`, `prax-secrets-proxy` cf86731) | 🟡 | **Boundary VERIFIED live 2026-08-08, auth unit-tested only.** What was measured on the dev box, before changing anything: the forward proxy is published to **127.0.0.1:8786 only** (not the network, despite the container binding `0.0.0.0`), and is **NOT reachable from the sandbox container** — probed over `127.0.0.1`, the docker bridge `172.17.0.1`, and `host.docker.internal`, all refused. So agent-generated code running via `run_python` could **not** reach it, and the real gap was host-local callers plus the absence of any per-caller attribution — not a remote hole. That is a measurement, not an assumption, and it is the reason this is 🟡 rather than 🔴. **What is NOT verified:** the new `Proxy-Authorization` enforcement has 14 unit tests (Basic + Bearer, constant-time compare, auth-before-rule-lookup so hosts can't be probed, credential stripped before forwarding, caller label in the audit line) but has **not** been exercised against a live mitmdump with a real client. Enforcement is opt-in so an existing deployment does not break on upgrade; the addon's `running` hook warns loudly when the token is unset. To verify: set `PROXY_FORWARD_AUTH_TOKEN`, `HTTPS_PROXY=http://prax:<token>@127.0.0.1:8786`, confirm a request succeeds and that omitting the credential returns 407. **The HTTPS path specifically is unverified** (added 2026-09): an HTTPS client presents `Proxy-Authorization` on the `CONNECT` request, while the addon reads the header in its `request` hook only (`secrets_proxy/mitm_addon.py` defines no `http_connect` hook) — whether real HTTPS callers are authenticated, or refused, has not been observed. |
...
| Surface | Status | Verified | Not verified / needs |
|---|---|---|---|
| `/v1/responses` through the secrets proxy + `previous_response_id` chaining | 🟡 | 2026-07-30: `build_llm(model="o4-mini")` with both flags against the live proxy — two sequential calls returned reasoning blocks; wiring confirmed, plus 6 keyless unit tests. 2026-08-02: a full **orchestrator turn** now runs end to end (6 A/B arm runs, 7/7 cases graded, 0 errored). | **Correction — this row previously read ✅ and was overstated.** What was verified on 07-30 was `build_llm().invoke()`, a bare model call; a full orchestrator turn had never run, and when the eval-gate A/B first tried one, *every* turn crashed (`TypeError: can only concatenate list (not "str") to list` — the Responses API returns list content blocks; fixed in `prax/agent/message_text.py`, #195). Lesson: verifying the layer you changed is not verifying the path the user takes. The **behavioral win** was then measured 2026-08-02 and is **NOT established**: 3 runs per arm, pass-rate means identical (0.809 vs 0.809), tokens lower with retention in all 3 pairs but p=0.125 — flags stay default-off and unrecommended ([full result](research/retained-reasoning-ab-2026-08-02.md)). |
...
## `prax.execute(JobSpec)` — the prax-lab handoff (`prax/exec_api.py`)

| Surface | Status | Verified | Not verified / needs |
|---|---|---|---|
| Schema validation, refusal semantics, timeline events, artifact hashing | ✅ | 40 keyless tests: every required field named on omission, unpinned `content_hash` refused, unimplemented `target.kind` and any `gpu>0` refused rather than silently downgraded, `rw` inputs refused, sandbox-absent refuses instead of falling back to the host, monotonic `seq` from 1, sha256 artifact hashes stable across runs. Live: prax-lab builds a real JobSpec, `validate_job_spec` accepts it, and `execute` refuses correctly with no sandbox. | — |
| Running a real job in the sandbox and collecting its artifacts | ✅ | **Verified live 2026-08-06** against the running sandbox, driving a prax-lab-built JobSpec end to end: `exit_code 0`, `error None`, `artifact_collection_error None`, **3 artifacts hashed and classified** (`metrics.json`→log 31B, `results.csv`→metric_csv 22B, `run.log`→log 15B), metrics extracted (`{"accuracy": 0.873, "f1": 0.81}`), and the full timeline emitted (`job_queued → job_running → log_chunk → 3× artifact_registered → metric_snapshot → job_succeeded`). Determinism confirmed: two separate jobs writing identical bytes produced the **same content_hash** with **distinct s3_keys**. | Two real bugs were found only by running it, both fixed: (1) `run_command` returns `subprocess.CompletedProcess`, not the dict the fakes returned; (2) **artifact collection was reading the wrong filesystem** — `run_command` executes in the container while the `file_list`/`file_read` API resolves per-user paths on the *host*, so outputs silently came back empty. Collection now reads the workdir back over the same exec channel the job ran in (tar+base64, so binary artifacts stay byte-exact), and Prax provisions the declared workdir. Not yet exercised: artifact sets near the 64 MB cap, and binary artifacts (checkpoints/parquet) specifically. |
...
## LLM request shaping (`prax/agent/loop_middleware.py`)
...
| Sandbox **desktop / noVNC** | 🔍 | Not assessed live. **Known gap (2026-09):** the `desktop_*` tools reach the display through `prax.utils.shell.run_command`, which routes into the container only when `RUNNING_IN_DOCKER=true` (`settings.sandbox_persistent`); on a native deployment (`make run-local-*`, the systemd unit) they run on the Prax host, where `desktop_open` executes a model-supplied command with `bash -c`. See `docs/architecture/hub-and-spoke.md` (Sandbox Agent). |
| **Twilio inbound webhook signature validation** (`prax/blueprints/twilio_auth.py`) | ⚪ | Never verified live behind the documented ngrok HTTPS tunnel; the ledger row above covers *outbound* Twilio only. Recorded 2026-09: validation is skipped entirely when `TWILIO_AUTH_TOKEN` is unset (one-time warning, requests pass), and when it *is* set the validator is given `request.url`, which is `http://…` behind a TLS-terminating tunnel while Twilio signed `https://…` — no `ProxyFix` / `X-Forwarded-Proto` handling exists in `app.py` or `prax/`, and the Twilio validator does not try the other scheme. Reproduced in-venv with werkzeug + `twilio.request_validator` (https-signed → `False` for the http URL); not observed against live Twilio. `tests/test_routes.py` blanks the token for every route test, so no test covers the signed path. |
| **Keyless mode for the direct `OpenAI(...)` clients** (`prax/plugins/capabilities.py` TTS/Whisper, `prax/conversation_memory.py`, `prax/services/youtube_service.py`, `prax/services/library_service.py`, `prax/services/sms_service.py`, `prax/readers/latex/*`, `prax/readers/web/web2mp3.py`, `prax/plugins/tools/image/plugin.py`) | 🔍 | Only `build_llm()` applies `OPENAI_BASE_URL`; these constructions pass no `base_url`, so under the base-URL-only reverse-proxy configuration (path B in `docs/security/secrets-proxy.md`) they would go straight to `api.openai.com` with the placeholder token. The `HTTPS_PROXY` forward-proxy configuration should cover them (the SDK's httpx transport honours the proxy variables), but that has not been checked per client either. Never exercised in base-URL-only mode. Recorded 2026-09. |
| **Nightly fresh-install workflow** (`.github/workflows/fresh-install.yml` — clean checkout of prax + sibling repos, sandbox image build, TeamWork SPA build, stack up + connectivity checks) | 🔴 | **Red since 2026-06-30.** `gh run list --workflow fresh-install.yml` on 2026-09-06: last success 2026-06-29; every scheduled run since has failed (69 failures out of the 77 runs listed). Until it is green again, the "fresh server" install path is **not** continuously verified — treat the README install instructions as unit-tested-only. Recorded 2026-09. |
| **MCP server** (`prax/mcp/`, default-off) | 🔍 | Real external-agent-over-MCP usage not confirmed here (has unit tests). |
| **`EMBEDDING_BASE_URL`** (openai embedding provider → local OpenAI-compatible server) | 🧪 | Unit-tested against a mocked client only (`tests/test_embedder_base_url.py`) — never exercised against a real local `/v1/embeddings` server (no GPU box here; llama.cpp-server CPU embeddings would verify it). Unset-path is bit-for-bit prior behaviour, so the risk is confined to the new opt-in. Added 2026-08-06 with the [SIE](research/sie-unified-local-inference.md) assessment. |
