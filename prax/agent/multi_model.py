"""Multi-model consensus — query several LLM providers and synthesize.

This capability used to live in the (now-retired) ``professor`` spoke,
whose name has been repurposed for the educator/teaching system.  The
multi-model consensus tool itself is still useful as a *research*
primitive, so it lives here as a standalone module that
``prax.agent.research_agent`` imports.

Use for: high-stakes research, fact verification, and nuanced analysis
where triangulating across providers (OpenAI, Anthropic, Google) catches
errors a single model would miss.
"""
...
def _query_single_model(
    ...
    logger.info("Multi-model querying %s", label)
    ...
    try:
        ...
        logger.info("Multi-model received response from %s (%d chars)", label, len(response))
        return {"provider": provider, "model": model, "response": response}
    except Exception as exc:
        logger.warning("Multi-model failed to query %s: %s", label, exc)
        return {
            "provider": provider,
            "model": model,
...
def _synthesize_responses(
    ...
    try:
        ...
        logger.warning("Multi-model synthesis failed: %s", exc)
...
def multi_model_query(question: str, models: str = "auto") -> str:
    ...
    logger.info(
        "Multi-model synthesizing %d responses (%d successful)",
